...
def measure_rois(mask_path, mask):
    ...
    ax.set_axis_off()
    plt.tight_layout()
    plt.savefig(f"../results/final_results/measurements_image_{mask_number}.png")
    plt.close(fig)
    measurements_df.to_csv(f"../results/final_results/measurements_file_{mask_number}.csv", index =False )
    print("------------------------")
