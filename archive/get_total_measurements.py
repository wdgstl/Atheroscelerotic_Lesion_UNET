...
if __name__ == "__main__":
    ...
    for x, y in tqdm(zip(test_x, test_y), total=2):
        ...
        mask = get_mask(x)
        measure_rois(x, mask)
        area = measure_rois_no_save(x, mask)
        end = datetime.datetime.now()
        elap = end - start
        print(f'total time: {elap} ')
        measurements.append({"Image": x, "Total Area": area})
        print({"Image": x, "Total Area": area})

    measurements_df = pd.DataFrame(measurements)
    measurements_df.to_csv(f"results/total_predicted_area_measurements.csv", index=False)